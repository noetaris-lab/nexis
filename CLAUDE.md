...
├── templates/
│   └── starlight/               # Bundled minimal Starlight project template (incl. astro-mermaid so wiki-page.md's Mermaid diagrams render; a `nexis-sidebar.mjs` the orchestrator rewrites for reading-order/Title-Case nav; `src/styles/nexis-wiki.css` widening the layout on big screens; `public/nexis-mermaid-zoom.js` for click-to-zoom/pan diagrams; and a `src/components/Pagination.astro` override that captions each Prev/Next link with its section so same-titled pages — every "Overview" — are distinguishable), copied + placeholder-patched by bootstrap-starlight.mjs
├── skills/
│   ├── ingest/
